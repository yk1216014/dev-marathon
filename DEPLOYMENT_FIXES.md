...
#### 修正後
ファイルパスを `path.join(__dirname, "web")` に修正しました。

#### 修正の理由・目的
Dockerコンテナのマウント設定と実際のディレクトリ構成において、`node` ディレクトリ直下に `web` ディレクトリが存在するため、`../`（一つ上の階層）を指定するとファイルが見つからない状態（File Not Found）になっていました。正しい相対パスに直すことで、画面を正常に表示させる目的です。
